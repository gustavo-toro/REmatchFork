...
Evaluator::inlinedNext(bool early_output, bool line_by_line) {
  ...
  while (!document_ended_) {
    ...
    while(((i_pos_-i_start_) < line_.size() &&  line_by_line) ||
          (i_pos_ < text_->size()             && !line_by_line)) { // Main search loop

      if(line_by_line)   a = line_[i_pos_-i_start_];
      else                text_->get(a);

      if(early_output)   readingT(a, i_pos_);
      else                readingF(a, i_pos_);

      current_states_.swap(new_states_);

      if(early_output)   captureT(i_pos_+1);
    ...
    if(early_output)   captureT(i_pos_);
    else                captureF(i_pos_);
    ...
    if(((i_pos_-i_start_) == line_.size()   &&  line_by_line) ||
       (i_pos_ == text_->size()             && !line_by_line)) {
      if(line_by_line_) {
